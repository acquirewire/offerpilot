...
bots (NTFY_SERVER, SMTP_HOST/SMTP_USER/SMTP_PASSWORD, ALERT_EMAIL_TO/FROM), so a
single .env drives every project. Both channels are best-effort — a failure is
logged, never raised, so the poll loop can't die on a bad alert.
"""
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            await client.post(server, json={"topic": topic, "title": title, "message": body,
                                            "priority": 4, "tags": ["money_with_wings"]})
